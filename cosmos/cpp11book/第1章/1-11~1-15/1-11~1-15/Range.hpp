...
// 稍做优化，用 value_t 替代了原来很多写死的int，规避 value_t to int 隐式转换的 warning
...
class RangeImpl {
   ...
   private:
    ...
    class Iterator {
       public:
        Iterator(value_t start, RangeImpl& range) : m_current_step(start), m_range(range) {
