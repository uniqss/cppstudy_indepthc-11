...
struct Complicated {
    ...
    Complicated(int a, double b, string c) : year(a), country(b), name(c) { cout << "is constructed" << endl; }

    Complicated(const Complicated& other) : year(other.year), country(other.country), name(other.name) {
        cout << "is moved" << endl;
    }
    ~Complicated() { cout << "~Complicated" << endl; }
};
...
int main(void) {
    ...
    cout << "这里展示了 map.emplace 和 vector.emplace_back 的强大之处 直接原地 construct 连move都省了" << endl;
